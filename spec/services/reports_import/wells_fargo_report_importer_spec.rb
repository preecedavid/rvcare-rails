...
module ReportsImport
  RSpec.describe WellsFargoReportImporter do
    let(:year) { Date.today.year }

    let!(:partner_report) do
      FactoryBot.create(:partner_report, year: year, type: 'WellsFargoReport')
    end

    describe '#call' do
      let(:dealer) { FactoryBot.create(:dealer, wells_fargo_account: 6.times.map { rand(10).to_s }.join) }
      ...
      context 'valid input data' do
        ...
        let(:input_data) do
          [
            { wells_fargo_account: dealer.wells_fargo_account, amount: 10, reported_on: "#{year}-04-10" },
            { wells_fargo_account: dealer.wells_fargo_account, amount: 17, reported_on: "#{year}-04-11" },
          ]
        end
      ...
      context 'wrong input data' do
        ...
        context 'wrong dealer_id' do
          ...
          let(:input_data) do
            [{ wells_fargo_account: 'unexistent account', amount: 10, reported_on: "#{year}-04-10" }]
          end
        ...
        context 'wrong year' do
          ...
          let(:input_data) do
            [
              { wells_fargo_account: dealer.wells_fargo_account, amount: 10, reported_on: "#{year-1}-04-10" },
              { wells_fargo_account: dealer.wells_fargo_account, amount: 17, reported_on: "#{year-1}-04-11" },
            ]
          end

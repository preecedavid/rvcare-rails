...
module ReportsImport
  RSpec.describe SalOtherReportImporter do
    let(:year) { Date.today.year }

    let!(:partner_report) do
      FactoryBot.create(:partner_report, year: year, type: 'SalOtherReport')
    end
    ...
    describe '#call' do
      ...
      context 'valid input data' do
        ...
        let(:input_data) do
          [
            ...
            { sal_account: dealer.sal_account, units: 17, reported_on: "#{year}-04-11" }
          ]
        end
        ...
        context 'report is uploaded the second time' do
          ...
          it "doesn't create new entries" do
            expect { subject.call }.not_to change(Units, :count)
          end
      ...
      context 'wrong input data' do
        ...
        %i[units reported_on].each do |absent_field|
        ...
        context 'wrong year' do
          ...
          let(:input_data) do
            [
              { sal_account: dealer.sal_account, units: 10, reported_on: "#{year - 1}-04-10" },
              { sal_account: dealer.sal_account, units: 17, reported_on: "#{year - 1}-04-11" }
            ]
          end
